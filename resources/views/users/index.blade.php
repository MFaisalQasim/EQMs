                                    ...
                                    <tbody>
                                        ...
                                        @if (Auth::User()->name == 'User A')
                                            @foreach ($users as $key => $user)
                                                @if ($user->Managed_by == 'User A' && (Auth::user()->name = $user->name))
                                                    <hr>
                                                    <tr>
                                                        <td>{{ $key + 1 }}</td>
                                                        <td>{{ $user->name }}</td>
                                                        ...
                                                        <th>
                                                            ...
                                                            <form action="/users-of-userb" method="get">

                                                                {{-- <a class="btn btn-success pull-right ml-3" type="submit"> View Users Under
                                                                    {{ $user->name }}
                                                                </a> --}}
                                                                <input type="hidden" name="user_name" value="{{ $user->name }}">
                                                                <button type="submit"class="btn btn-success pull-right ml-3"
                                                                {{-- href="/users-of-userb" --}} type="submit"> View Users Under
                                                                {{ $user->name }}
                                                                </button>
                                                            </form>
                                                        </th>
